...
class SlalomLocomotionTask(DirectRLEnv):
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # action reward
        action_reward = torch.sum(torch.square(self.robot.data.applied_torque), dim=1) * self.cfg.action_weight
        
        # energy reward
        energy_reward = torch.sum(self.robot.data.applied_torque *self.robot.data.joint_vel , dim=1) * self.cfg.electricity_weight
        # dof at limit reward
        dof_at_limit_reward = torch.sum(self.dof_pos_scaled > 0.98, dim=-1)  * self.cfg.dof_at_limit_weight
        ...
        rewards = {
            ...
            "electricity_reward" : energy_reward ,
            "dof_at_limit_reward" : dof_at_limit_reward ,
        }
